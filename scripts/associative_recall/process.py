...
if __name__ == "__main__":
    ...
    for batch_dir in os.listdir("data"):
        ...
        for exp_dir in os.listdir(os.path.join("data", batch_dir)):
            ...
            if not os.path.exists(os.path.join("data", batch_dir, exp_dir, "network_results.csv")):
                print(f"Results not found at {os.path.join('data', batch_dir, exp_dir, 'network_results.csv')}", flush=True)
                continue

            df = pd.read_csv(os.path.join("data", batch_dir, exp_dir, "network_results.csv"))
            print(f"Loaded {os.path.join('data', batch_dir, exp_dir, 'network_results.csv')}", flush=True)
            list_of_network_df.append(df)
    ...
    pd.concat(list_of_network_df).to_csv(os.path.join("data", "network_results.csv"), index=False)
    print(f"Results saved to {os.path.join('data', 'network_results.csv')}", flush=True)
