...
list_of_dict = []
for l in trange(100):
    for std_jitter in tqdm(np.logspace(-2, 1, 6)):
        neuron = load_object_from_file(os.path.join("neurons_", f"neuron_{l}_l2_{std_jitter}.pkl"))
        corr, _ = single_channel_correlation(ref_firing_times[l], neuron.firing_times[neuron.firing_times >= 0], PERIOD, SIMILARITY_KERNEL)
        list_of_dict.append({"neuron": l, "std_jitter": std_jitter**2, "corr": corr})
...
fig.tight_layout()
# plt.show()
fig.savefig('jitters_disturbance.pdf', bbox_inches='tight', pad_inches=0.01, dpi=300)
