...
from .utils import all_close_to_one_of, obs_block_


def solve_lp(A, b, xb, gamma_y=1.0, gamma_xb=1.0, max_iter=1000, rtol=1e-5, atol=1e-8, device="cpu"):
    """
    Solve the linear program subject.

    Args:
        A (torch.tensor): the constraint matrix with shape (N, K).
        b (torch.tensor): the constraint vector with shape (N).
        xb (float): the variable bound.
    ...
    if device == "mps" and torch.backends.mps.is_available():
        A = A.to(device="mps")
        b = b.to(device="mps")
        mxf = torch.empty(K, device="mps")
    else:
        mxf = torch.empty(K)

    # init with random posterior means
    mxf.uniform_(-xb, xb)  # (K)
    prev_mxf = mxf
    ...
    for i in trange(max_iter):
        ...
        Vxf = torch.diag(Vxf)  # (K, K)
        for n in range(N):
            obs_block_(mxf, Vxf, myb[n], Vyb[n], A[n])  # inplace operation, (K) and (K, K)

        # check convergence
        if torch.allclose(mxf, prev_mxf, rtol=rtol, atol=atol):
            summary = {
                "num_iter": i,
                "status": "converged",
                "constraints": torch.all(A @ mxf <= b),
                "bounds": torch.all(torch.abs(mxf) <= xb),
                "x": mxf,
            }
            return summary
...
#         A (torch.tensor): the constraint matrix with shape (N, K).
#         b (torch.tensor): the constraint vector with shape (N).
#         xb (torch.tensor): the variable bounds with shape (K).
#         gamma_y (float, optional): the constraints strength parameter. Defaults to 1.0.
#         gamma_xb (float, optional): the bounds strength parameter. Defaults to 1.0.
